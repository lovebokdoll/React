...
const KhSearchBar = () => {
  ...
  const[tTitle, setTTitle]= useState('제목');//제목, 내용, 작성자 중에 한가지
  ...
  useEffect(() => {
	...
	search.split('&').forEach((item)=>{
		...
		//요청 url에 담긴 condition정보를 setTTitle
		if(item.match('condition')){
			setTTitle(item.split('=')[1])
		}
	})
  },[search, setTTitle]) //의존배열에 seach를 사용했고  상태훅을 선택했으니 그 정보가 변경될때마다 호출
  ...
  const setPath = () => {
	...
	console.log(search)
	//자바스크립트에서는 싱글 혹은 더블로 묶지 않으면 변수취급함
	console.log(search.match('condition'))
    let path;
	//앞에서 조회한 적이 있을 때 기존에 쿼리스트링 삭제후 다시 쿼리스트링 만들어야 함
